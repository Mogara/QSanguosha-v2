    ...
    bool handcard_visible, Card::HandlingMethod method, const QList<int> &disabled_ids, bool obtain, bool wanwei)
{
    ...
    if ((!flags.contains("j") || who->getCards("j").isEmpty()) && wanwei && (who->hasSkill("wanwei") || who->getMark("wanwei") != 0) && askForSkillInvoke(who, "wanwei")) {
    ...
    thread->trigger(ChoiceMade, this, player, decisionData);
     if (wanwei && (who->hasSkill("wanwei") || who->getMark("wanwei") != 0) && (getCardPlace(card_id) == Player::PlaceHand || getCardPlace(card_id) == Player::PlaceEquip) && askForSkillInvoke(who, "wanwei"))
     {
        broadcastSkillInvoke("wanwei");
