...
public:
    ...
    bool trigger(TriggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        ...
        if (damage.card && damage.card->isKindOf("Slash") && damage.by_user && !damage.chain && !damage.transfer
            ...
            if ((who->hasSkill("wanwei") || who->getMark("wanwei") != 0) && (room->getCardPlace(card_id) == Player::PlaceHand || room->getCardPlace(card_id) == Player::PlaceEquip) && room->askForSkillInvoke(who, "wanwei"))
            {
