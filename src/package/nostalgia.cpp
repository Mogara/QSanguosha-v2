#include "standard.h"
#include "skill.h"#include "standard.h"
#include "skill.h"
#include "wind.h"
#include "client.h"
#include "engine.h"
...
public:
    ...
    bool trigger(TriggerEvent, Room *room, ServerPlayer *yueying, QVariant &data) const
    {
        ...
        if (use.card->isNDTrick() && (yueying->getMark("JilveEvent") > 0 || room->askForSkillInvoke(yueying, objectName()))) { 
        ...
        if (room->askForSkillInvoke(zhouyu, objectName())) {
            ...
            if (!zhouyu->hasInnateSkill(this)) {
                if (zhouyu->hasSkill("hunzi"))
                    ...
                    index += 2;
                else if (zhouyu->hasSkill("jiazhou"))
                    index += 5;
            }
